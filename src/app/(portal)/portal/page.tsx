...
import { updateMediaStatus, getPortalMediaFiles } from "@/actions/media";
import { revalidatePath } from "next/cache";
import { cn } from "@/lib/utils";
import { decrypt } from "@/lib/encryption";
import { PortalMediaGrid } from "@/components/portal/portal-media-grid";
...
export default async function PortalDashboardPage() {
  ...
  const [mediaResponse, contentPosts, domains, credentials] = await Promise.all([
    // Media files
    getPortalMediaFiles({
      page: 1,
      limit: 6,
      projectIds: isClient ? clientProjectIds : undefined,
    }),
    // Calendar posts
    !isClient
      ? supabase
  ...
  const clientMediaFiles = mediaResponse.data || [];
  const clientMediaCount = mediaResponse.count || 0;
  ...
  return (
    <div className="p-6 md:p-8 space-y-12 max-w-7xl mx-auto animate-in fade-in duration-200">
      ...
      <section className="space-y-4">
        ...
        <PortalMediaGrid
          initialFiles={clientMediaFiles}
          initialCount={clientMediaCount}
          clientProjectIds={isClient ? clientProjectIds : []}
        />
      </section>
