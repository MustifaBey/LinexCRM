...
export async function createTicket(data: {
  ...
  try {
    ...
    if (staff) {
      const notificationsToInsert = (staff as any[]).map((s) => ({
        user_id: s.id,
        title: "Yeni Destek Talebi",
        message: `Müşteri yeni bir destek talebi açtı: "${data.subject}"`,
        type: "info",
        action_url: `/tickets`,
      }));
...
export async function createTicketMessage(data: {
  ...
  // Notify relevant user(s) about the new ticket message
  try {
      ...
      if (data.isStaff) {
        ...
        if (ticket.client_id) {
          await supabase.from("notifications").insert({
            ...
            message: `Destek talebinize yeni bir yanıt yazıldı: "${ticket.subject}"`,
            type: "info",
            action_url: `/portal/tickets`,
          } as any);
        }
      } else {
        // Client replied, notify all staff members (except sender)
        const { data: staff } = await supabase
          .from("profiles")
          .select("id")
          .in("role", ["owner", "admin", "member"]);
        ...
        if (staff) {
          const notificationsToInsert = (staff as any[])
            .filter((s) => s.id !== user.id)
            .map((s) => ({
              user_id: s.id,
              title: "Destek Talebine Yeni Yanıt",
              message: `Destek talebine yeni yanıt: "${ticket.subject}"`,
              type: "info",
              action_url: `/tickets`,
            }));
          if (notificationsToInsert.length > 0) {
    ...
    console.error("Failed to insert ticket message notification:", notifErr);
  }
