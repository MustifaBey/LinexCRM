...
export function UpdateNotifier() {
  ...
  useEffect(() => {
    ...
    // Support test simulation event
    const handleSimulateUpdate = (e: Event) => {
      const customEvent = e as CustomEvent<{ type: string; progress?: number }>;
      const { type, progress: percent } = customEvent.detail;
      ...
      if (type === "available") {
        ...
        setProgress({
          percent: percent || 0,
          transferred: (percent || 0) * 10000,
          total: 1000000,
          bytesPerSecond: 5242880, // 5 MB/s
        });
    ...
    window.addEventListener("simulate-update", handleSimulateUpdate);
    ...
    if (electron) {
      unsubAvailable = electron.onUpdateAvailable((info: unknown) => {
        console.log("Yeni güncelleme mevcut:", info);
        setUpdateAvailable(true);
        setDownloaded(false);
      });

      unsubProgress = electron.onUpdateProgress((progressObj: ProgressData) => {
        console.log("İndirme ilerlemesi:", progressObj);
        setUpdateAvailable(true);
        setProgress(progressObj);
      });

      unsubDownloaded = electron.onUpdateDownloaded((info: unknown) => {
    ...
    return () => {
      ...
      window.removeEventListener("simulate-update", handleSimulateUpdate);
    };
  }, []);
  ...
  const handleRestart = () => {
    if (isSimulating) {
      toast.success("Uygulama yeniden başlatılıyor ve güncelleniyor");
      setUpdateAvailable(false);
      setDownloaded(false);
      setProgress(null);
      setIsSimulating(false);
