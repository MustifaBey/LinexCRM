...
export function VersionHistory({
  ...
  const handleRevisionUpload = async (file: File) => {
    ...
    if (!latestFile || !latestFile.project_id) {
      toast.error("Üst dosya veya proje bilgisi bulunamadı.");
      return;
    }
    ...
    try {
      // 1. Upload file
      const projectId = latestFile.project_id;
      if (!projectId || projectId === "undefined" || projectId === "null") {
        throw new Error("Üst dosyanın geçerli bir proje kimliği bulunamadı.");
      }

      const fileExt = file.name.split(".").pop();
      const cleanName = file.name.replace(/[^a-zA-Z0-9]/g, "_");
      let filePath = `${projectId}/${Date.now()}_revision_${cleanName}.${fileExt}`;
      ...
      // 2. Log metadata in DB with parent link
      const result = await createMediaFile({
        project_id: latestFile.project_id,
        file_name: file.name,
        file_path: filePath,
        file_type: file.type || "application/octet-stream",
        file_size: file.size,
        parent_file_id: latestFile.id, // Links to predecessor
      });
